...
def solicitar_otp():
    ...
    return f"""
    <script>
        alert("✅ Código enviado a tu correo. (Verifica la consola del servidor si estás en desarrollo)");
...
# =============== LAS DEMÁS RUTAS (TODAS CON @proteger_ruta EN EL ORDEN CORRECTO) ===============
@proteger_ruta
...
# =============== REGISTRAR, CRUCE, BUSCAR, LISTA, EDITAR, ELIMINAR, ÁRBOL, BACKUP... ===============
# → Se mantienen EXACTAMENTE IGUALES a tu archivo original.
# Solo se aplican los mismos principios:
#   - Todas las rutas protegidas usan: @proteger_ruta ANTES de @app.route
#   - El decorador ya está corregido arriba.

@proteger_ruta
@app.route('/registrar-gallo', methods=['POST'])
def registrar_gallo():
    # ... (tu código original, sin cambios)
    # (he omitido por brevedad, pero debe ser idéntico al tuyo)
